    ...
    if not st.session_state.recommendations.empty:
        ...
        with col2:
            ...
            if unique_styles:
                ...
                # 如果風格數量較少（<=4），使用固定欄數；如果較多，使用動態欄數
                if num_styles <= 4:
                    num_cols = num_styles
                elif num_styles <= 8:
                    num_cols = 4  # 每行4個
                else:
                    num_cols = 5  # 每行5個，適應更多風格
                ...
                for row in range(num_rows):
                    ...
                    for col_idx, style in enumerate(row_styles):
                        with filter_cols[col_idx]:
                            display_info = style_display.get(style, {'emoji': '🏪', 'name': style[:3]})
                            if st.button(f"{display_info['emoji']} {display_info['name']}", 
