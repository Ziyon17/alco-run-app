...
<style>
    ...
    .stButton > button {
        ...
        width: 100% !important; /* 確保按鈕寬度100% */
        min-width: auto !important; /* 移除最小寬度限制 */
        font-size: 14px !important; /* 確保字體大小 */
        padding: 8px 12px !important; /* 確保內邊距 */
    }
    ...
    /* 提示框樣式 - 顯示在圖標下方，確保不會超出側邊欄邊界 */
    .info-icon .tooltip {
        visibility: hidden;
        width: 220px; /* 減少寬度避免超出邊界 */
        background-color: #1a202c !important;
        color: #fff;
        text-align: left;
        border-radius: 6px;
    ...
    /* 向上指向的箭頭 - 調整位置對應右對齊的提示框 */
    .info-icon .tooltip::before {
        content: "";
        position: absolute;
        bottom: 100%;
        left: 15px; /* 調整箭頭位置，對齊圖標 */
        border-width: 0 6px 8px 6px;
        border-style: solid;
        border-color: transparent transparent #4a5568 transparent;
    }
...
def generate_time_options():
    """生成固定的時間選項"""
    # 開始時間選項：18:00-次日3:00AM，每隔一個小時
    start_options = []
    
    # 當天的時間選項 (18:00-23:00)
    for h in range(18, 24):
        start_options.append(f"{h:02d}:00")
    
    # 次日的時間選項 (00:00-03:00)
    for h in range(0, 4):
        if h == 0:
            start_options.append("次日 00:00")
        else:
            start_options.append(f"次日 {h:02d}:00")
    
    # 結束時間選項：19:00-次日4:00AM，每隔一個小時
    end_options = []
    
    # 當天的時間選項 (19:00-23:00)
    for h in range(19, 24):
        end_options.append(f"{h:02d}:00")
    
    # 次日的時間選項 (00:00-04:00)
    for h in range(0, 5):
        if h == 0:
            end_options.append("次日 00:00")
        else:
            end_options.append(f"次日 {h:02d}:00")
...
def calculate_advanced_score(bar, preferences):
    ...
    # 風格匹配 (25% 權重)
    bar_styles_pref = preferences.get('bar_styles', {})
    
    # 檢查是否選擇了「沒有偏好」
    if bar_styles_pref.get('沒有偏好', False):
        # 如果選擇了「沒有偏好」，給予滿分
        score += 0.25
    else:
        # 正常風格匹配邏輯
        selected_styles = [k for k, v in bar_styles_pref.items() if v and k != '沒有偏好']
    ...
    # 音樂匹配 (20% 權重)
    music_types_pref = preferences.get('music_types', {})
    
    # 檢查是否選擇了「沒有偏好」
    if music_types_pref.get('沒有偏好', False):
        # 如果選擇了「沒有偏好」，給予滿分
        score += 0.20
    else:
        # 正常音樂匹配邏輯
        selected_music = [k for k, v in music_types_pref.items() if v and k != '沒有偏好']
...
def get_unique_bar_styles(recommendations):
    """提取推薦結果中的所有唯一酒吧風格"""
    if recommendations.empty:
        return []
    
    all_styles = set()
    for _, bar in recommendations.iterrows():
        if bar['bar_style'] != 'N/A':
            styles = [style.strip() for style in str(bar['bar_style']).split(',')]
...
def create_interactive_map(recommendations, show_styles=None):
    ...
    for idx, bar in recommendations.iterrows():
        ...
        bar_styles_list = []
        if bar['bar_style'] != 'N/A':
            bar_styles_list = [style.strip() for style in str(bar['bar_style']).split(',')]
        
        # 風格篩選
        if show_styles:
            should_show = False
            for style in bar_styles_list:
                if style in show_styles:
                    should_show = True
                    break
            if not should_show:
                continue
        ...
        # 創建詳細彈出窗口
        popup_html = f"""
        <div style="font-family: Arial, sans-serif; width: 400px; padding: 12px;">
            <div style="background: linear-gradient(135deg, {unified_color}, {unified_color}aa); color: white; padding: 12px; margin: -12px -12px 12px -12px; border-radius: 5px 5px 0 0;">
                <h3 style="margin: 0; font-size: 18px;">{idx+1}. {bar['final_name']}</h3>
            </div>
        ...
        if pd.notna(bar.get('top_3_selection')) and bar['top_3_selection'] != 'N/A':
            ...
            drinks_html = '<br>'.join([f"• {drink.strip()}" for drink in drinks])
            popup_html += f"""
                <div style='margin: 10px 0; padding: 10px; background: #f5f5f5; border-radius: 5px; border-left: 3px solid {unified_color};'>
                    <strong>🍹 人氣酒單:</strong><br>
                    <div style='font-size: 12px; color: #333; margin-top: 6px; line-height: 1.4;'>{drinks_html}</div>
                </div>
            """
        ...
        if pd.notna(bar.get('formatted_phone_number')):
            popup_html += f'<div style="margin-top: 12px;"><strong>📞 電話:</strong> {str(bar["formatted_phone_number"])}</div>'
        ...
        folium.Marker(
            ...
            tooltip=f"{idx+1}. {bar['final_name']} ({', '.join(bar_styles_list)})",
            icon=folium.DivIcon(
                html=f'''
                <div style="
                    font-size: 14px; 
...
def main():
    ...
    # 生成時間選項
    start_options, end_options = generate_time_options()
    ...
    with st.sidebar:
        ...
        with st.expander("⏰ 時間設定", expanded=True):
            # 時間選項
            start_time_index = 1  # 默認選擇19:00 (18:00是index 0，19:00是index 1)
            start_time = st.selectbox("開始時間", start_options, index=start_time_index)
            
            end_time_index = 4  # 默認選擇23:00 (19:00是index 0，23:00是index 4)
            end_time = st.selectbox("結束時間", end_options, index=end_time_index)
        ...
        with st.expander("🍷 酒吧風格偏好", expanded=True):
            ...
            # 添加「沒有偏好」選項
            bar_style_selections['沒有偏好'] = st.checkbox('沒有偏好', key="bar_no_preference")
        ...
        with st.expander("🎵 音樂風格偏好", expanded=False):
            ...
            # 添加「沒有偏好」選項
            music_selections['沒有偏好'] = st.checkbox('沒有偏好', key="music_no_preference")
    ...
    if not st.session_state.recommendations.empty:
        ...
        with col2:
            ...
            # 動態風格篩選器標題和顯示全部按鈕
            unique_styles = get_unique_bar_styles(st.session_state.recommendations)
            ...
            if unique_styles:
                ...
                with button_col:
                    # 使用 success 類型的按鈕來區分顏色
                    if st.button("🔍 顯示全部", type="primary", use_container_width=True, key="show_all_bars"):
                        selected_filter = None  # 顯示所有酒吧
            else:
                st.subheader("🎛️ 地圖篩選器")
            
            if unique_styles:
                # 根據風格數量自動調整欄數，確保按鈕文字完整顯示
                num_styles = len(unique_styles)
                
                # 減少每行的按鈕數量，給更多空間顯示完整文字
                if num_styles <= 3:
                    num_cols = num_styles
                elif num_styles <= 6:
                    num_cols = 3  # 每行3個，給更多空間
                else:
                    num_cols = 4  # 每行4個，平衡空間和顯示
                ...
                # 風格名稱對應的emoji和顯示名稱
                style_display = {
                    '夜店型酒吧': {'emoji': '🌃', 'name': '夜店型酒吧'},
                    '立飲酒吧': {'emoji': '🍺', 'name': '立飲酒吧'},
                    '餐酒館': {'emoji': '🍽️', 'name': '餐酒館'},
                    '精緻酒吧': {'emoji': '✨', 'name': '精緻酒吧'},
                    '啤酒專門店': {'emoji': '🍻', 'name': '啤酒專門店'},
                    '威士忌酒吧': {'emoji': '🥃', 'name': '威士忌酒吧'},
                    '茶酒酒吧': {'emoji': '🍵', 'name': '茶酒酒吧'},
                    '咖啡餐酒館': {'emoji': '☕', 'name': '咖啡餐酒館'},
                    '音樂酒吧': {'emoji': '🎵', 'name': '音樂酒吧'},
                    '運動酒吧': {'emoji': '⚽', 'name': '運動酒吧'},
                    '露天酒吧': {'emoji': '🌙', 'name': '露天酒吧'},
                    '主題酒吧': {'emoji': '🎭', 'name': '主題酒吧'},
                    '調酒吧': {'emoji': '🍸', 'name': '調酒吧'},
                    '清酒吧': {'emoji': '🍶', 'name': '清酒吧'}
                }
                ...
                for row in range(num_rows):
                    ...
                    end_idx = min(start_idx + num_cols, num_styles)
                    row_styles = unique_styles[start_idx:end_idx]
                    
                    # 為這一行創建欄位，確保有足夠空間
                    filter_cols = st.columns(len(row_styles))
                    
                    for col_idx, style in enumerate(row_styles):
                        with filter_cols[col_idx]:
                            display_info = style_display.get(style, {'emoji': '🏪', 'name': style})
                            # 確保使用完整的名稱
                            button_text = f"{display_info['emoji']} {display_info['name']}"
                            if st.button(button_text, 
                                       use_container_width=True, 
                                       key=f"filter_{style}_{row}_{col_idx}"):
                                selected_filter = [style]
            else:
                st.info("🔍 生成推薦路線後，將顯示相應的風格篩選器")
