...
try:
    ...
    options = get_dynamic_options(df)
...
# Extract unique options from data for dynamic forms
@st.cache_data
def get_dynamic_options(df):
    # Get unique bar styles
    bar_styles = []
    for styles in df['bar_style'].dropna():
        bar_styles.extend([s.strip() for s in styles.split(',')])
    unique_bar_styles = sorted(list(set(bar_styles)))
    
    # Get unique music types  
    music_types = []
    for music in df['music_type'].dropna():
        music_types.extend([m.strip() for m in music.split(',')])
    unique_music_types = sorted(list(set(music_types)))
    
    # Get environment types (0=室外, 1=室內, 2=兩者皆可)
    env_type_mapping = {0: "室外", 1: "室內", 2: "兩者皆可"}
    
    return {
        'bar_styles': unique_bar_styles,
        'music_types': unique_music_types,
        'env_mapping': env_type_mapping
    }
...
# 4. Bar style preferences (now using dynamic data)
st.subheader("酒吧風格")
st.write("請選出今晚的酒精路跑，希望在什麼風格的酒吧進行呢？（可複選）")

# Create dynamic checkboxes based on actual data
bar_style_selections = {}
if data_loaded and 'bar_styles' in options:
    # Create columns for better layout (adjust number based on available options)
    cols = st.columns(3)
    for i, style in enumerate(options['bar_styles']):
        with cols[i % 3]:
            bar_style_selections[style] = st.checkbox(style)
else:
    # Fallback to static options if data loading fails
    col1, col2, col3 = st.columns(3)
    with col1:
...
# 5. Music preferences (now using dynamic data)
st.subheader("音樂風格")
st.write("請選出今晚的酒精路跑，希望搭配的是甚麼音樂風格呢？（可複選）")

# Create dynamic checkboxes based on actual data
music_selections = {}
if data_loaded and 'music_types' in options:
    # Create columns for better layout
    cols = st.columns(3)
    for i, music_type in enumerate(options['music_types']):
        with cols[i % 3]:
            music_selections[music_type] = st.checkbox(music_type, key=f"music_{music_type}")
else:
